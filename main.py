...
output_dir = Path('/home1/08524/jqfang/scratch/model_2411/2d/')
basic_dir = output_dir / 'basic'
velocity_dir = output_dir / 'velocity'
viscosity_dir = output_dir / 'viscosity'
...
if(uw.mpi.rank == 0):
    ...
    viscosity_dir.mkdir(exist_ok=True, parents=True)
...
pt_vars = getGlobalSwarmVar(np.hstack((swarm.data,
                                        materialIndex.data,
                                        temperatureField.evaluate(swarm),
                                        shearModulusFn.evaluate(swarm),
                                        frictionSt.evaluate(swarm),
                                        frictionDyn.evaluate(swarm),
                                        refSlipVelocity.evaluate(swarm),
                                        porePressureFactor.evaluate(swarm),
                                        normalStress.evaluate(swarm))))
if(uw.mpi.rank == 0):
    np.savez(basic_dir / f'vars_0.npz', loc=pt_vars[:, :mesh.dim], vars=pt_vars[:, mesh.dim:])

# Solve long-term (creating prestress IC for short-term)
if(restart_step < -1):
    flag = 'l'
...
# Save long-term viscosity
pt_vars = getGlobalSwarmVar(np.hstack((swarm.data,
                                        materialIndex.data,
                                        viscosityFn.evaluate(swarm))))
if(uw.mpi.rank == 0):
    np.savez(viscosity_dir / f'viscosity.npz', loc=pt_vars[:, :mesh.dim], vars=pt_vars[:, mesh.dim:])
...
while(step < nsteps_short):
    ...
    if(step >= 100):
        ...
        pressureField.save(str(velocity_dir / f'pressure_{flag}{step:d}.h5'))
        previousStress.save(str(velocity_dir / f'prestress_{flag}{step:d}.h5'))
